...
int studentsClass[membersSize];        // Classes of Students

// Loading and Storing Data into Files
string
parseData(string line, int field);
...
void viewStudents();
void addStudent(string student, int grade, string username, string password, string rollnumber);
...
int main()
{
    ...
    int choice, grade;
    string masterUser, masterPass, username, password, role1, username1, password1, name, rollnumber;
    while (mainLoop)
    {
        clearScreen();
        loginScreen();
...
void viewStudents()
{
    cout << "List of Members of Library" << endl
         << endl;
    cout << left << setw(20) << "Student Names" << setw(20) << "Class" << setw(20) << "RollNumber" << endl;
    for (int i = 0; i < userIndex; i++)
    {
        cout << left << setw(20) << students[i] << setw(20) << studentsClass[i] << setw(20) << studentRollNumber[i] << endl;
    }
    cout << endl;
    halt();
}
...
void loadMembersDataToArrays()
{
    ...
    while (!file.eof())
    {
        if (getline(file, line))
        {
            ...
            string rollnumber = parseData(line, 2);
            string grade = parseData(line, 3);
            string userName = parseData(line, 4);
            string userPassword = parseData(line, 5);
            string role = parseData(line, 6);
            userNames[userIndex] = userName;
            userPasswords[userIndex] = userPassword;
            roles[userIndex] = role;
            studentRollNumber[userIndex] = rollnumber;
            students[userIndex] = name;
            studentsClass[userIndex] = stoi(grade);
            userIndex++;
        }
    }
    file.close();
...
void storeMembersDataToFile()
{
    ...
    for (int i = 0; i < userIndex; i++)
    {
        file << students[i] << "," << studentRollNumber[i] << "," << studentsClass[i] << "," << userNames[i] << "," << userPasswords[i] << "," << roles[i] << endl;
    }
    file.close();
}
void loadBooksDataToArrays()
{
    ...
    while (!file.eof())
    {
        if (getline(file, line))
        {
            ...
            string bookstatus = parseData(line, 5);
            string issuedStudentName = parseData(line, 6);
            string dueInDate = parseData(line, 7);
            bookIds[indexbook] = bookId;
            books[indexbook] = book;
            authors[indexbook] = author;
            genres[indexbook] = genre;
            status[indexbook] = bookstatus;
            issuedStudentNames[indexbook] = issuedStudentName;
            dueInDates[indexbook] = dueInDate;
            indexbook++;
        }
    }
    file.close();
...
void storeBooksDataToFile()
{
    ...
    for (int i = 0; i < indexbook; i++)
    {
        file << bookIds[i] << "," << books[i] << "," << authors[i] << "," << genres[i] << "," << status[i] << "," << issuedStudentNames[i] << "," << dueInDates[i] << endl;
    }
    file.close();
}